...
interface Field {
    ...
    type: "text" | "select"; // Kiểu input: text hoặc select
...
const SearchFilters: React.FC<SearchFiltersProps> = ({
      ...
      case "select":
        return (
          <Select
            key={field.field}
            placeholder={`Select ${field.label}`}
            value={searchParams[field.field] || undefined}
            onChange={(value) => onSearchChange(field.field, value)}
            style={{ width: 120 }}
          >
            {field.options?.map((option) => (
  ...
  // Sử dụng useMemo để tối ưu hóa re-render
  ...
  return (
    <div className="bg-white p-2 rounded-lg m-2">
      <h2 className="font-bold text-xl my-3">Search</h2>
      <Space direction="horizontal">{renderedFields}</Space>
    </div>
  );
};
//<-
